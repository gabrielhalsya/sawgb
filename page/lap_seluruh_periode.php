...
<div class="container">
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <?php $no = 1; ?>
            <?php if ($query = $connection->query("SELECT COUNT(alternatif.nik) as pelamar, periode.* from alternatif,periode where alternatif.kd_periodes=periode.kd_periode GROUP BY periode.kd_Periode")): ?>
                <?php while($row = $query->fetch_assoc()): ?>
                <tr>
                    <td><?=$no++?></td>
                    <td><?=$row['pelamar']?> orang</td>
                    <td><?=$row['nama']?></td>
